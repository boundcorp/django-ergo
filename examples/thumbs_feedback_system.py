...
class SimpleThumbsFeedback:
    ...
    def get_user_vote(self, response_id: str, user_id: str) -> Optional[bool]:
        """
    ...
    def get_learning_insights(self, time_period: Optional[str] = None) -> Dict[str, Any]:
        """
        ...
        return {
            ...
            'common_positive_patterns': self._extract_common_patterns(positive_feedback),
            'common_negative_patterns': self._extract_common_patterns(negative_feedback),
...
def demonstrate_thumbs_feedback():
    ...
    response1_id = feedback.create_trackable_response(
        ...
        sources=[
            {'id': 'chunk_1', 'content': 'Python installation guide...', 'source': 'python.org'},
            {'id': 'chunk_2', 'content': 'Download Python installer...', 'source': 'docs.python.org'}
        ],
        user_id="user123"
    )
    ...
    response2_id = feedback.create_trackable_response(
        ...
        sources=[
            {'id': 'chunk_3', 'content': 'ML definition...', 'source': 'ml_textbook.pdf'},
        ],
        user_id="user456"
    )
    ...
    print(f"Response 1 - Score: {stats1['score']}, Approval: {stats1['approval_rate']:.1%}")
    
    stats2 = feedback.get_feedback_stats(response2_id)
    print(f"Response 2 - Score: {stats2['score']}, Approval: {stats2['approval_rate']:.1%}")
