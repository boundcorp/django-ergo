...
from examples.thumbs_feedback_system import SimpleThumbsFeedback
from examples.thumbs_feedback_system import ThumbsVote
...
class TestSimpleThumbsFeedback:
    ...
    def test_thumbs_up_vote(self):
        ...
        stats = self.feedback.thumbs_up(
            self.response_id, "user123", comment="Very helpful!"
        )
    ...
    def test_thumbs_down_vote(self):
        ...
        stats = self.feedback.thumbs_down(
            self.response_id, "user123", comment="Not helpful at all"
        )
    ...
    def test_needs_review_flag(self):
        ...
        stats = self.feedback.get_feedback_stats(self.response_id)
        assert stats["needs_review"] is True  # More downs than ups with >= 3 votes
    ...
    def test_get_user_vote(self):
        ...
        # After thumbs up
        self.feedback.thumbs_up(self.response_id, "user123")
        assert self.feedback.get_user_vote(self.response_id, "user123") is True

        # After changing to thumbs down
        self.feedback.thumbs_down(self.response_id, "user123")
        assert self.feedback.get_user_vote(self.response_id, "user123") is False
    ...
    def test_learning_from_positive_feedback(self):
        ...
        with (
            patch.object(self.feedback, "_boost_source_score") as mock_boost,
            patch.object(self.feedback, "_record_successful_pattern") as mock_pattern,
        ):
    ...
    def test_learning_from_negative_feedback(self):
        ...
        with (
            patch.object(self.feedback, "_decrease_source_score") as mock_decrease,
            patch.object(self.feedback, "_flag_problematic_pattern") as mock_flag,
        ):
    ...
    def test_learning_recommendations(self):
        ...
        for i in range(5):
            response_id = self.feedback.create_trackable_response(
                query=f"Question {i}", llm_response=f"Answer {i}", sources=[]
            )
        ...
        negative_rec = any(
            "negative than positive" in rec.lower() for rec in recommendations
        )
        assert negative_rec
    ...
    def test_responses_needing_review(self):
        ...
        bad_response_id = self.feedback.create_trackable_response(
            query="Bad question", llm_response="Bad answer", sources=[]
        )
        ...
        assert needing_review[0]["stats"]["needs_review"] is True
