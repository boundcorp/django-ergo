...
class TestWorkflowEngineOpenAI(TestCase):
    ...
    def setUp(self):
        ...
        self.mock_openai_patcher = patch("django_ergo.workflow_engine.openai")
        self.mock_openai = self.mock_openai_patcher.start()
        mock_client = MagicMock()
        mock_response = MagicMock()
        mock_response.choices = [MagicMock()]
        mock_response.choices[0].message.content = "Mocked assistant response"
    ...
    def test_current_development_mode(self):
        """Test that workflow engine returns a response via OpenAI."""
    ...
    def test_workflow_engine_with_real_openai_when_activated(self):
        ...
        with patch("django_ergo.workflow_engine.openai") as mock_openai:
            ...
            mock_response.choices[
                0
            ].message.content = "Django is a Python web framework..."
            mock_response.choices[0].message.tool_calls = None
    ...
    def test_workflow_engine_with_mocked_openai(self):
        ...
        with patch("django_ergo.workflow_engine.openai") as mock_openai:
            ...
            response_message = self.engine.process_message(
                self.chat, fixture.input_data["message"]
            )
    ...
    def test_build_conversation_history(self):
        ...
        self.chat.add_message(
            message_type=MessageType.USER_INPUT, content="Hello", role=MessageRole.USER
        )
        ...
        context = WorkflowContext(
            user=self.user, chat=self.chat, workflow=self.workflow
        )

        # _build_conversation_history returns chat messages without system message
        ...
        # _build_openai_conversation_history prepends the system message
        openai_messages = self.engine._build_openai_conversation_history(context)
        assert len(openai_messages) >= 4
        assert openai_messages[0]["role"] == "system"
        assert openai_messages[0]["content"] == self.workflow.instructions
...
class TestWorkflowEngineToolCalls:
    ...
    def setUp(self):
        """Set up test data."""
        self.user = User.objects.create_user(
            username="tooluser", email="tool@example.com", password="testpass123"
        )
        ...
        self.chat = UserChat.objects.create(
            user=self.user, workflow=self.workflow, title="Tool Chat"
        )
    ...
    def test_tool_calls_real_api(self):
        ...
        with patch("django_ergo.tool_registry") as mock_registry:
            ...
            mock_response.choices[
                0
            ].message.content = "I'll search for Python web frameworks."
            ...
            mock_response.choices[0].message.tool_calls[
                0
            ].function.arguments = '{"query": "Python web frameworks"}'
            mock_response.usage = Mock()
            mock_response.usage.model_dump.return_value = {
                "prompt_tokens": 30,
...
class TestWorkflowEngineContextManagement:
    ...
    def test_workflow_context_creation(self):
        ...
        context = WorkflowContext(
            user=user, chat=chat, workflow=workflow, state={"step": 1, "data": "test"}
        )
    ...
    def test_workflow_context_default_state(self):
        ...
        context = WorkflowContext(user=user, chat=chat, workflow=workflow)
