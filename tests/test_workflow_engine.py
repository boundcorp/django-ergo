...
from tests.openai_test_utils import save_openai_fixture
...
class TestWorkflowEngineCore(TestCase):
    ...
    def test_get_tool_whitelist(self):
        ...
        empty_workflow = Workflow.objects.create(
            name="Empty Workflow", instructions="No tools"
        )
...
class TestWorkflowEngineSignals(TestCase):
    ...
    def test_tool_approval_requested_signal(self):
        ...
        try:
            ...
            with (
                patch.object(engine, "_build_conversation_history", return_value=[]),
                patch.object(engine, "serialize_workflow_context", return_value={}),
            ):
                engine._create_approval_request(
                    context, Mock(), Mock(), pending_approvals
                )
...
class TestWorkflowEngineOpenAIIntegration(TestCase):
    ...
    def test_tool_approval_flow_real_api(self):
        ...
        chat = UserChat.objects.create(
            user=self.user, workflow=workflow, title="Approval Test Chat"
        )
