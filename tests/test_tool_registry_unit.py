...
class MockToolRegistry:
    ...
    def register_tool(self, name: str, description: str, requires_approval: bool = False, readonly: bool = False):
    ...
    def get_tool(self, name: str) -> Optional[MockToolConfig]:
        """Get tool configuration by name."""
        return self._tools.get(name)
    
    def execute_tool(self, name: str, user, arguments: Dict[str, Any], approved: bool = False):
        ...
        if config.requires_approval and not approved:
            raise ValueError(f"Tool '{name}' requires approval but was not approved")
        
        func = self._tool_functions.get(name)
        if not func:
            raise ValueError(f"Function for '{name}' not found")
