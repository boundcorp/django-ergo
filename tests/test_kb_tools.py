...
from django_ergo.kb_tools import send_email_notification
...
class TestSearchUserKB(TestCase):
    ...
    def test_search_user_kb_success(self, mock_article_objects):
        ...
        results = search_user_kb(user=self.user, query="Python programming", top_k=5)
        ...
        self.assertEqual(
            result1["content"], "Python is a high-level programming language."
        )
    ...
    def test_search_user_kb_no_results(self):
        ...
        results = search_user_kb(user=empty_user, query="nonexistent", top_k=5)
    ...
    def test_search_user_kb_with_kb_name_filter(self, mock_article_objects):
        ...
        results = search_user_kb(
            user=self.user, query="Python", kb_name="Test Knowledge", top_k=3
        )
...
class TestSearchGardenKB(TestCase):
    ...
    def test_search_garden_kb_success(self, mock_article_objects):
        ...
        mock_queryset = Mock()
        mock_article_objects.filter.return_value = mock_queryset
        mock_queryset.hybrid_search.return_value = [
            self.garden_article,
            self.plant_article,
        ]

        results = search_garden_kb(user=self.user, query="plant care", top_k=5)
    ...
    def test_search_garden_kb_no_garden_kbs(self):
        ...
        Knowledgebase.objects.filter(name__icontains="garden").delete()
        Knowledgebase.objects.filter(name__icontains="plant").delete()

        results = search_garden_kb(user=self.user, query="garden tips", top_k=5)
...
class TestGetKBTableOfContents(TestCase):
    ...
    def test_get_kb_table_of_contents_success(self):
        ...
        result = get_kb_table_of_contents(user=self.user, kb_name="Test Knowledge Base")
    ...
    def test_get_kb_table_of_contents_not_found(self):
        ...
        result = get_kb_table_of_contents(user=self.user, kb_name="Nonexistent KB")
...
class TestGetArticleByHierarchy(TestCase):
    ...
    def test_get_article_by_hierarchy_success(self):
        ...
        result = get_article_by_hierarchy(
            user=self.user, kb_name="Test Knowledge Base", hierarchy_code="A1"
        )
    ...
    def test_get_article_by_hierarchy_not_found(self):
        ...
        result = get_article_by_hierarchy(
            user=self.user, kb_name="Nonexistent KB", hierarchy_code="A1"
        )
        ...
        result = get_article_by_hierarchy(
            user=self.user, kb_name="Test Knowledge Base", hierarchy_code="Z9"
        )
...
class TestDeleteUserArticle(TestCase):
    ...
    def test_delete_user_article_success(self):
        ...
        result = delete_user_article(user=self.user, article_id=article_id)
        ...
        self.assertEqual(
            result["deleted_article"]["knowledgebase"], "Test Knowledge Base"
        )
    ...
    def test_delete_user_article_errors(self):
        ...
        result = delete_user_article(
            user=self.user, article_id="00000000-0000-0000-0000-000000000000"
        )
        ...
        result = delete_user_article(
            user=self.user, article_id=str(self.other_article.id)
        )
...
class TestToolRegistration(TestCase):
    ...
    def test_kb_tools_registered(self):
        ...
        for tool_name in readonly_tools:
            ...
            self.assertTrue(
                tool_config.readonly, f"Tool {tool_name} should be readonly"
            )
    ...
    def test_tool_execution_through_registry(self):
        ...
        result = tool_registry.execute_tool(
            name="list_user_knowledgebases", user=user, arguments={}, approved=True
        )
        ...
        kb = Knowledgebase.objects.create(
            name="Test KB", description="Test", owner_id=str(user.id)
        )
