...
from examples.rag_examples import HybridSearchEngine
from examples.rag_examples import SmartDocumentChunker
...
def knowledge_base():
    ...
    return Knowledgebase.objects.create(
        name="test_kb", description="Test knowledge base"
    )
...
class TestSmartDocumentChunker(TestCase):
    ...
    def test_respects_token_limits(self):
        ...
        for chunk in chunks:
            self.assertLessEqual(chunk["token_count"], self.chunker.config.max_tokens)
    ...
    def test_overlap_handling(self):
        ...
        if len(chunks) > 1:
            ...
            for i in range(len(chunks) - 1):
                ...
                self.assertTrue(any(word in chunk2_start for word in chunk1_end))
...
class TestHybridSearchEngine(TestCase):
    ...
    def setUp(self):
        self.kb = Knowledgebase.objects.create(name="test_kb", description="Test KB")
    ...
    def test_reranking(self):
        ...
        title_match_1 = self.engine._calculate_title_match(
...
class TestContextWindowOptimizer(TestCase):
    ...
    def test_basic_optimization(self):
        ...
        result = self.optimizer.optimize_context(
            query="test query", retrieved_chunks=chunks, required_tokens=200
        )
    ...
    def test_respects_token_limits(self):
        ...
        result = self.optimizer.optimize_context(
            query="test", retrieved_chunks=chunks, required_tokens=200
        )

        # First chunk (500 tokens) fits in 800 available, second gets truncated to 300
        self.assertEqual(result["num_chunks"], 2)
        self.assertLessEqual(result["total_tokens"], 800)
    ...
    def test_chunk_truncation(self):
        ...
        result = self.optimizer.optimize_context(
            query="test", retrieved_chunks=chunks, required_tokens=100
        )
...
class TestFeedbackLearningSystem(TestCase):
    ...
    def test_interaction_recording(self, mock_get):
        """Test recording interactions"""
        mock_article = Mock()
        mock_article.relevance_score = 1.0
        mock_get.return_value = mock_article

        chunks = [{"id": 1, "content": "Test content", "score": 0.9}]
    ...
    @patch("examples.rag_examples.Article.objects.get")
    def test_feedback_pattern_analysis(self, mock_get):
...
class TestRAGPipelineIntegration(TestCase):
    ...
    def test_end_to_end_pipeline(self):
        ...
        context = optimizer.optimize_context(
            query="How to install?", retrieved_chunks=results, required_tokens=200
        )
        ...
        with patch("examples.rag_examples.Article.objects.get") as mock_get:
            mock_article = Mock()
            mock_article.relevance_score = 1.0
            mock_get.return_value = mock_article
