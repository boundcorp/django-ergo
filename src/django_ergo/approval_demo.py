...
from django_ergo.workflow_engine import workflow_paused
...
def demonstrate_approval_workflow():
    ...
    chat = UserChat.objects.create(
        user=user, workflow=workflow, title="Tool Approval Demo Chat"
    )
    ...
    message1 = workflow_engine.process_message(
        chat=chat, message_content="Please search my knowledge base for 'django'"
    )
...
def demonstrate_whitelisting():
    ...
    print(
        f"✅ Strict workflow: {len(workflow_engine.get_tool_whitelist(strict_workflow))} whitelisted tools"
    )
    print(
        f"✅ Permissive workflow: {len(workflow_engine.get_tool_whitelist(permissive_workflow))} whitelisted tools"
    )
    ...
    print(
        f"  Strict workflow allows: {workflow_engine.is_tool_whitelisted(strict_workflow, test_tool)}"
    )
    print(
        f"  Permissive workflow allows: {workflow_engine.is_tool_whitelisted(permissive_workflow, test_tool)}"
    )
