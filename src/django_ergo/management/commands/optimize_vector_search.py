...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--vacuum", action="store_true", help="Run VACUUM ANALYZE on vector tables"
        )
        parser.add_argument(
            "--query", type=str, help="Analyze performance of a specific search query"
        )
    ...
    def analyze_query(self, query_text):
        ...
        if index_info.get("index_name"):
            ...
            self.stdout.write(
                f"  • Startup cost: {index_info.get('startup_cost', 'N/A')}"
            )
