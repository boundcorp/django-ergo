...
from django_ergo.models import Workflow, Knowledgebase, Article, UserChat
...
Tools can be configured as read-only or require approval. The system supports both automatic execution and user approval workflows.'''
        ...
        for article_data in sample_articles:
            ...
            if created:
                self.stdout.write(f'Created article: {article.hierarchy_code} - {article.title}')
            else:
                self.stdout.write(f'Using existing article: {article.hierarchy_code} - {article.title}')
...
Most herbs prefer well-drained soil and moderate watering. Many can be grown in containers.'''
        ...
        for article_data in garden_articles:
            ...
            if created:
                self.stdout.write(f'Created garden article: {article.hierarchy_code} - {article.title}')
