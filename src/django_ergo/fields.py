...
def vector_search(model_class, vector_field_name: str, query_vector: List[float], top_k: int = 10):
    ...
    return (
        model_class.objects
        .exclude(**{f"{vector_field_name}__isnull": True})
        .annotate(semantic_distance=CosineDistance(vector_field_name, query_vector))
        .order_by("semantic_distance")[:top_k]
    )


def semantic_search(model_class, vector_field_name: str, query_text: str, top_k: int = 10):
    """
...
class SemanticTextField(models.TextField):
        ...
        if has_changed:
            try:
                ...
                if hasattr(model_instance, embedding_field_name):
                    ...
                    print(f"Warning: No embedding field '{embedding_field_name}' found for SemanticTextField '{self.attname}'")
                
            except Exception as e:
                print(f"Error processing SemanticTextField '{self.attname}': {e}")
    ...
    @classmethod
    def search_field(cls, model_class, field_name: str, query_text: str, top_k: int = 10):
        """
    ...
    @classmethod
    def search_field_vector(cls, model_class, field_name: str, query_vector: List[float], top_k: int = 10):
        """
