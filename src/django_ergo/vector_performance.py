...
class VectorIndexManager:
    ...
    # Index configuration
    HNSW_M = getattr(settings, 'DJANGO_ERGO_HNSW_M', 16)  # Number of connections for HNSW
    HNSW_EF_CONSTRUCTION = getattr(settings, 'DJANGO_ERGO_HNSW_EF_CONSTRUCTION', 64)  # Search effort during construction
    IVFFLAT_LISTS = getattr(settings, 'DJANGO_ERGO_IVFFLAT_LISTS', 100)  # Number of clusters for IVFFlat
    ...
    def get_index_info(cls) -> Dict[str, Any]:
        ...
        with connection.cursor() as cursor:
            ...
            return {
                ...
                "recommended_maintenance": cls._get_maintenance_recommendations(table_stats)
            }
    ...
    def _get_maintenance_recommendations(cls, table_stats) -> List[str]:
        ...
        if dead_tuples > live_tuples * 0.1:  # More than 10% dead tuples
            recommendations.append("Consider running VACUUM on django_ergo_article table")
    ...
    def _get_performance_tips(cls, query_plan: Dict) -> List[str]:
        ...
        def analyze_node(node):
            ...
            if node.get("Node Type") == "Seq Scan":
                tips.append("Sequential scan detected - ensure vector indexes are created and used")
            
            # Check for expensive operations
            if node.get("Total Cost", 0) > 1000:
                tips.append("High-cost query detected - consider optimizing search parameters")
            
            # Check memory usage
            if node.get("Peak Memory Usage", 0) > 100000:  # 100MB
                tips.append("High memory usage - consider reducing top_k or adding more selective filters")
